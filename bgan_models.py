...
class BGAN(object):
    def __init__(self, x_dim, z_dim, dataset_size, batch_size=64, prior_std=1.0, J=1, M=1,
        ...
        for name, dim in self.weight_dims.iteritems():
            self.sghmc_noise[name] = tf.contrib.distributions.Normal(mu=0., sigma=self.noise_std * tf.ones(
...
class BDCGAN(BGAN):
    def __init__(self, x_dim, z_dim, dataset_size, batch_size=64, gf_dim=64, df_dim=64,
        ...
        print("S_h16", s_h16)
        print("S_h", s_h)
        print("gf dim", self.gf_dim)
        print("df dim", self.df_dim)
        print("c dim", self.c_dim)

        self.gen_params = AttributeDict()
        self.bgen_params = AttributeDict()
        self.weight_dims = OrderedDict([("g_h0_lin_W", (self.z_dim, self.gf_dim * 4 * s_h16 * s_w16)),
                                        ("g_h0_lin_b", (self.gf_dim * 4 * s_h16 * s_w16,)),
                                        ("g_h1_W", (5, 5, self.gf_dim * 2, self.gf_dim * 4)),
                                        ("g_h1_b", (self.gf_dim * 2,)),
                                        ("g_h2_W", (5, 5, self.gf_dim * 1, self.gf_dim * 2)),
                                        ("g_h2_b", (self.gf_dim * 1,)),
                                        ("g_h3_W", (5, 5, self.c_dim, self.gf_dim * 1)),
                                        ("g_h3_b", (self.c_dim,))])
    ...
    def discriminator(self, image, K, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            h3 = linear(tf.reshape(h2, [self.batch_size, -1]), K, 'd_h2_lin')
    ...
    def generator(self, z, gen_params):
        with tf.variable_scope("generator") as scope:
            ...
            self.z_, self.h0_w, self.h0_b = linear(z, self.gf_dim * 4 * s_h16 * s_w16, 'g_h0_lin', with_w=True,
                                                   matrix=gen_params.g_h0_lin_W, bias=gen_params.g_h0_lin_b)

            self.h0 = tf.reshape(self.z_, [-1, s_h16, s_w16, self.gf_dim * 4])
            h0 = tf.nn.relu(self.g_bn0(self.h0))

            self.h1, self.h1_w, self.h1_b = deconv2d(h0,
                                                     [self.batch_size, s_h4, s_w4, self.gf_dim * 2], name='g_h1',
                                                     with_w=True,
    ...
    def sampler(self, z, gen_params):
        with tf.variable_scope("generator") as scope:
            ...
            # project `z` and reshape
            z_ = linear(z, self.gf_dim * 4 * s_h16 * s_w16, 'g_h0_lin',
                        matrix=gen_params.g_h0_lin_W, bias=gen_params.g_h0_lin_b)

            h0 = tf.reshape(z_, [-1, s_h16, s_w16, self.gf_dim * 4])
